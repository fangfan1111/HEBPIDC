...
<div class="nav-tabs-custom">
	...
	<form:form id="inputForm" modelAttribute="testAudit" action="${ctx}/oa/testAudit/save" method="post" class="form-horizontal">
		...
		<div class="form-actions">
			<input id="btnCancel" class="btn pull-right" type="button" value="返 回" onclick="history.go(-1)"/>
			<shiro:hasPermission name="oa:testAudit:edit">
				<input id="btnSubmit" class="btn btn-primary pull-right" type="submit" value="提交申请" onclick="$('#flag').val('yes')"/>&nbsp;
				<c:if test="${not empty testAudit.id}">
					<input id="btnSubmit2" class="btn btn-inverse pull-right" type="submit" value="销毁申请" onclick="$('#flag').val('no')"/>&nbsp;
				</c:if>
			</shiro:hasPermission>
		</div>
		<c:if test="${not empty testAudit.id}">
