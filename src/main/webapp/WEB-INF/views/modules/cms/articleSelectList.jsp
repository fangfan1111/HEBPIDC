...
<body>
	...
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/article/selectList" method="post" class="breadcrumb form-horizontal">
		...
		<sys:treeselect id="category" name="category.id" value="${article.category.id}" labelName="category.name" labelValue="${article.category.name}"
					title="栏目" url="/cms/category/treeData" module="article" cssClass="form-control input-sm"/>
					</div>
	    ...
	    <input id="btnSubmit" class="btn btn-primary" type="button" onclick="closes();" value="关闭"/>&nbsp;&nbsp;
