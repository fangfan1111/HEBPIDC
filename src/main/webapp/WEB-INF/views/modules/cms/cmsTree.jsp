...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var zNodes=[
		            <c:forEach items="${categoryList}" var="tpl">{id:'${tpl.id}', pId:'${not empty tpl.parent?tpl.parent.id:0}', name:"${tpl.name}", url:"${ctx}/cms/${tpl.module ne 'e4e7d7723abe4e24a6364ee07b7d87f4' and empty tpl.href ?'article':'none'}/?category.id=${tpl.id}", target:"cmsMainFrame"},
		            </c:forEach>];
...
// 			$(".ztree").width($(window).width()-16).height($(window).height()-62);
			$(".ztree").width($(window).width()-16).height(height-62);
